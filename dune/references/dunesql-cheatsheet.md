...
For cross-chain tables (e.g. `dex.trades`, `nft.trades`), also add a `blockchain` filter — this further reduces scan size:

```sql
-- GOOD: blockchain + time filter for cross-chain tables
...
Dune uses columnar storage. Selecting `SELECT *` reads all columns; selecting only what you need reduces I/O significantly.
...
Break complex queries into CTEs. They improve readability and let the engine optimize each step independently.
...
Include the **partition column filter in the `ON` clause** (not just `WHERE`) when joining two large partitioned tables. This ensures both sides are pruned before the join.

```sql
-- GOOD: partition filter in ON clause prunes both tables before join
SELECT t.hash, t.block_time, l.topic0, l.data
FROM ethereum.transactions t
INNER JOIN ethereum.logs l
    ON t.hash = l.tx_hash
    AND t.block_date = l.block_date          -- join on partition column
